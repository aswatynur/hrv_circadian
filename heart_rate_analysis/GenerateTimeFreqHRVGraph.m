...
dir_plots = 'P:\HR_Main_Research_2024\plots\';
...
dir_cosinor = 'P:\HR_Main_Research_2024\';
            ...
            for d_seq=1:length(durations)
                ...
                for col = 1:numCols
                    ...
                    if isnumeric(hrv_table{:, col})
                        ...
                        dirPlot = strcat(dir_plots, 'mouse-', mouse_seq(c_seq), '\ga_', num2str(h_seq), '\', hrv_types{type_seq}, '_', num2str(duration), 'minutes\');
...
for d_seq = 1:length(durations)
    duration = durations(d_seq);
    writetable(T_cosinor(T_cosinor.Duration==duration, 1:end), strcat(dir_cosinor, num2str(duration), '-minutes-cosinor.csv'));
end
